...
public:
    ...
    bool colision(int fx, int fy){
        ...
        //Si toca un ladrillo
            ...
            if(mapa[y][fx] != 0) rebota(fx,y);
			else if(mapa[fy][x] != 0) rebota(x,fy);
			else if(mapa[fy][fx] != 0) rebota(fx,fy);
            else if(mapa[fx][fy] != 0) rebota(fy, fx);
...
void inputmov(){
    if(GetAsyncKeyState(VK_LEFT) & 0x8000){ //Mascara que consigue el bit mas significativo y verifica realmente que se esta precionando la tecla
        paddle.dir = 'L';
        Sleep(10);
    ...
    if (ball.direccion == 4) ball.direccion = rand()%2;
}
...
void display(){
	...
	recorreXY(2,1); cout << "Vidas: " << vidas;
    recorreXY(22,1); cout << "Puntos: " << puntos;
    // 9=Laterales 8=Puntaje 7=Superior 5=Pelota 2=Bloques 1=Barra	
	for(int i = 0; i < screenHeight; i++){
		for(int k = 0; k < screenWidth; k++){
			recorreXY(k+2, i+3);
			if(mapa[i][k] == 9) cout << char(219);
			if(mapa[i][k] == 8) cout << char(240);
			if(mapa[i][k] == 7) cout << char(219);
			if(mapa[i][k] == 5) cout << char(254);
			if(mapa[i][k] == 2) cout << char(233);
			if(mapa[i][k] == 1) cout << char(219);
			if(mapa[i][k] == 0) cout << char(32);
		}
	}
}
